...
public class Apriori
{
	...
	public ArrayList<Integer> getAnswer(String inputStr, Set<String> inputSet){
		String[] qa = inputStr.split(";");
		TreeSet<String> qs = new TreeSet<>(Arrays.asList(qa));
		int flag=3;
		ArrayList<Integer> answer = new ArrayList<>();
		System.out.println("========================================");
		for(int i=qa.length;i>=1;i--) {
			if(flag<=0) {
				break;
			}
			for (String rrKey : inputSet)
			{
				if(flag<=0) {
					break;
				}
				String[] tempStrArray1 = rrKey.substring(0, (rrKey.indexOf("->"))).split(";");
				if(tempStrArray1.length==i) {
					System.out.println("||||||------"+i+"------||||||");
					if(qs.containsAll(new TreeSet<>(Arrays.asList(tempStrArray1)))) {
						String[] tempStrArray2 = rrKey.substring((rrKey.indexOf("->")+2)).split(";");
						for(int j=0;j<tempStrArray2.length;j++) {
							if(inputStr.indexOf(tempStrArray2[j])==-1) {
								System.out.println("推荐："+tempStrArray2[j]);
								if(!answer.contains(Integer.parseInt(tempStrArray2[j]))) {
									if(flag--<=0) {
										break;
									}
									answer.add(Integer.parseInt(tempStrArray2[j]));
								}
							}
						}
	...
	public static void main(String[] args)
	{
		...
		dataList.add("1;5;6;7;8;");
		dataList.add("2;4;7;9");
		dataList.add("2;3;8;10;");
		dataList.add("2;4;7;9");
		dataList.add("3;6;10;");
		dataList.add("2;7;3;9");
		dataList.add("1;6;3;10;");
		dataList.add("10;2;3;7;8;");
		dataList.add("1;2;6;9;");
		dataList.add("2;3;6;");
		dataList.add("2;3;7;10;");
		dataList.add("1;4;6;8;9;");
		dataList.add("5;7;10;");
		dataList.add("4;6;5;8;");
