			...
			function selectMap(){
				...
				$.ajax({
					...
					success:function(data){
						var html = "";
						html+="<table>";
						for( var i = 0; i < data.graArray.length; i++ ) {
						    html += "<tr>";
							for(var j = 0; j < data.graArray[i].length;j++ ){
								if(data.graArray[i][j] == -1){
									html +=     "<td><button class=\"btn\" id=\"btn"+data.graArray[i][j]+"\"   value=\""+data.graArray[i][j]+"\" style=\"background-color:yellow;width:100%;height:100%\">"+"**"+"</button></td>";
								}else{
									html +=     "<td><button class=\"btn points\" id=\"btn"+data.graArray[i][j]+"\" style=\"width:100%;height:100% \" value=\""+data.graArray[i][j]+"\">"+data.graArray[i][j]+"</button></td>";
								}
						...
						$(".points").click(function(){
							...
							if(spotsArray.indexOf($(this).val()) != -1){
								spotsArray.pop($(this).val());
								$(this).css("background-color","");
							}else{
								spotsArray.push($(this).val());
				...
				$.ajax({
					...
					success:function(data){
						for(var i=0;i<data.route.length;i++){
							if(spotsArray.indexOf(data.route[i].toString()) == -1){
								$('#btn'+data.route[i]).css("background-color","blue");
							}	
						}
					},
					error:function(){
		...
		<div class="container-fluid content">	
			<div class="row">
				...
				<div class="main" >
					...
					<button id="button1" class="btn" onclick = "selectMap()" value="" >确定</button>
					<button id="button2" class="btn" onclick = "setMap()">查询路径</button>
						<div id = "gra">
